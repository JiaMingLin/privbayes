...
public class PrivTool {
	...
	public static <T> T ExpoMech(Random rng, HashMap<T, Double> solutions, double epsilon, double delta){
		// 	Exponential mechanism: sample solution i with a probability proportional 
		//	to exp(epsilon*utility[i]/2 delta)
		
		double sum = 0.0;
		
		double maxu = Collections.max(solutions.values());
		double balance = 10 - maxu*epsilon/(2*delta);				//avoid precision error (inf or 0) when epsilon is large
